...
switch ($op){
  case 'issubscribe': 
	if($_SCOOKIE['wxopenid']){
	  $wxopenid = addslashes($_SCOOKIE['wxopenid']);
  ...
  case 'lottery': 
	...
	if (($lotteryusenums >= $lottery['canrqnums']) and ($lotteryusenums>0)) {
	  ...
	  if ($prizetype<7) {	
		  //中奖了
		  $sn = uniqid();
		  /*$sql="insert into ".$_SC['tablepre']."lotteryrecord (uid,lotteryid,wxopenid,prizetype,phone,sn,ip,dateline) value (".$user['uid'].",".$lottery['id'].", '{$wxopenid}','{$prizetype}','','{$sn}','".getonlineip()."',{$_SGLOBAL['timestamp']})";
		  $query = $_SGLOBAL['db']->query($sql);*/
