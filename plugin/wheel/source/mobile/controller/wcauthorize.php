...
switch ($op){
  case 'getcode': 
		...
		$lotteryid=$_SGET['lotteryid']?$_SGET['lotteryid']:'';
	  $redirect_uri= $_SCONFIG['siteallurl']."plugin/".$_PSC['name']."/index-wcauthorize-op-getaccesstoken-id-".$lotteryid.".html";
  ...
  break;
	case 'getaccesstoken': 
		...
		if(!empty($tq_openid)){
			...
			header("Location:".$_PSPATH['plugroot']."index-index-lotteryid-{$id}.html");
		}
		exit;	
  break;
  default:
