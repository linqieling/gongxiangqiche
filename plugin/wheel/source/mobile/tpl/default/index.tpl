...
<div class="main">
  ...
  <div class="content">
	...
	<div class="boxcontent boxyellow">
    <div class="box">
      <div class="title-red">用户信息：</div>
      <div class="Detail">
        <div style="float: left;width:30%;text-align: center;"><img style="width: 70px;height: 70px;" src="[##if $user.avatar##][##$user.avatar##][##else##][##$_SPATH.images##]face/noavatar_middle.gif[##/if##]" alt=""></div>
        <div style="float: left;width:70%; text-align: left;">
          <div>你好，<span style="color:blue;">[##if $user##][##$user.nickname##][##else##]游客[##/if##]</span></div>
          <div>每人最多允许抽奖:
						<span id="canrqnums">[##$result.canrqnums##]</span>次 [##if $user##]- 
						您还有<span id="slotteryusenums" class="red">[##$result.canrqnums-$lotteryusenums##]</span>次抽奖机会 - 
						当前您已抽取 <span class="red" id="lotteryusenums">[##$lotteryusenums|default:0##]</span> 次
						[##else##]<br>
						请先关注公众号
						[##/if##]
...
$(function() {
    ...
    $("#inner").click(function() {
	   ...
	   $.ajax({
         url     : "[##$_PSPATH.plugroot##]index-index-lotteryid-[##$lotteryid##]-op-lottery.html",
         dataType: "json",
         type    : "POST",
  ...
  $("#save-btn").click(function() {
	...
	$.ajax({
		 url     : "[##$_PSPATH.plugroot##]index-index-lotteryid-[##$lotteryid##]-op-lotteryadd.html",
		 dataType: "json",
		 type    : "POST",
		 ...
		 success    : function(data){
			if (data.success == true) {
				layer.open({
				  ...
				  btn: ['关闭'],
				  yes: function(){
					  location.href = "[##$_PSPATH.plugroot##]index-index-lotteryid-[##$lotteryid##].html";
				  }
				});
				$("#result").hide("slow");
				return
