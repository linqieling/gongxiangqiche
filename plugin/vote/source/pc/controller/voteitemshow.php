...
switch ($op){
		...
		if($_GET['id']){
			...
			if($_SCOOKIE['openid']){
				...
				if(count($user)){
					$subscribe = checksubscribe($openid);
					if($subscribe==1){
						$sql="select * from ".$_SC['tablepre'].$_PC['tablepre']."voteitem where id='{$id}'";
				        $query = $_SGLOBAL['db']->query($sql);
				        $voteitem = $_SGLOBAL['db']->fetch_array($query);
						if(count($voteitem)){
							$sql="select * from ".$_SC['tablepre'].$_PC['tablepre']."vote where id='{$voteitem[voteid]}'";
				            $query = $_SGLOBAL['db']->query($sql);
							...
							if($vote['starttime']>$_SGLOBAL['timestamp']){
								...
								$sql="select * from ".$_SC['tablepre'].$_PC['tablepre']."voterecord where uid={$user[uid]} and voteid={$vote[id]} and voteday={$timedate} order by id desc";
				                $query = $_SGLOBAL['db']->query($sql);
								...
								if($vote['ipnubs']>0){
									$sql="select * from ".$_SC['tablepre'].$_PC['tablepre']."voterecord where ip='".addslashes($ip)."' and voteid={$vote[id]} and voteday={$timedate} order by id desc";
									$query = $_SGLOBAL['db']->query($sql);
									while ($value = $_SGLOBAL['db']->fetch_array($query)) {  
									...
									if(count($voterecord_group_ip)<$vote['ipnubs']){
										if(count($voterecord_group)<$vote['tpnub']){
											if($vote['tpxznub']){
												$sql="select * from ".$_SC['tablepre'].$_PC['tablepre']."voterecord where uid={$user[uid]} and voteid={$vote[id]} and itemid={$voteitem[id]} and voteday={$timedate} order by id desc";
												$query = $_SGLOBAL['db']->query($sql);
												while ($value = $_SGLOBAL['db']->fetch_array($query)) {  
													$voterecord[]=$value;
												}
											...
											if($tpxznub==1){//判断用户是否已经给这个用户投过一票
											    ...
											    $voterecordid = inserttable($_SC['tablepre'].$_PC['tablepre'],"voterecord", $data, 1 );
												if($voterecordid){
													//投票成功
													$data['status']=108;
													$sql = "update ".$_SC['tablepre'].$_PC['tablepre']."voteitem set votenum=votenum+1 where id={$voteitem[id]}";
													$query = $_SGLOBAL['db']->query($sql);
												}else{
  ...
  default:
	...
	if(!empty($id)){
	   $sql="select voteitem.* from ".$_SC['tablepre'].$_PC['tablepre']."voteitem as voteitem
			 left join  ".$_SC['tablepre'].$_PC['tablepre']."vote as vote on voteitem.voteid=vote.id
			 where voteitem.id=".$id;
	   $query = $_SGLOBAL['db']->query($sql);
	   $result = $_SGLOBAL['db']->fetch_array($query);
	   
	   //用户是否已经投过票了
	   $sql = "select * from ".$_SC['tablepre'].$_PC['tablepre']."voterecord where voteid={$result[id]} and cookies='".$_SCOOKIE['votecookies']."' and dateline>=".strtotime(date("Y-m-d",$_SGLOBAL['timestamp']));
	   $query = $_SGLOBAL['db']->query($sql);
	   $voterecord = $_SGLOBAL['db']->fetch_array($query);
	   
	   if($result['voteid']){
		 $sql="select * from ".$_SC['tablepre'].$_PC['tablepre']."vote where id=".$result['voteid'];
		 $query = $_SGLOBAL['db']->query($sql);
		 $vote = $_SGLOBAL['db']->fetch_array($query);
	   }	   
	   //计算用户的排名
	   $sql="select count(*) from ".$_SC['tablepre'].$_PC['tablepre']."voteitem where votenum>{$result[votenum]}";
	   $rank = $_SGLOBAL['db']->result($_SGLOBAL['db']->query($sql), 0);
	   $rank = $rank+1;
	   //前一名的票数
	   $sql="select votenum from ".$_SC['tablepre'].$_PC['tablepre']."voteitem where votenum>{$result[votenum]} order by votenum asc";
	   $prevotenum = $_SGLOBAL['db']->result($_SGLOBAL['db']->query($sql), 0);
	   //用户有没有上传过作品
	   $openid = addslashes($_SCOOKIE['openid']);
	   $sql="select * from ".$_SC['tablepre'].$_PC['tablepre']."voteitem where voteid={$vote[id]} and openid='{$openid}'";
	   $query = $_SGLOBAL['db']->query($sql);
