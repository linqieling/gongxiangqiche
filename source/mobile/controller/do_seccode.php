...
//配置
$seccodedata = array (
	'width' => empty($_SGET['width'])?'50':intval($_SGET['width']),
	'height' =>  empty($_SGET['height'])?'25':intval($_SGET['height']),
	'adulterate' => '1',//随机背景图形
	'angle' => '1',//随机倾斜度
	'shadow' => '1',//阴影
);
...
//设定cookie
ssetcookie('seccode', tq_authcode($seccode, 'ENCODE'));
...
function seccode_background() {
	...
	for($i = 0;$i < 3;$i++) {
		$start[$i] = mt_rand(255, 255);
		$end[$i] = mt_rand(255, 255);
	...
	for($i = 0;$i < $seccodedata['width'];$i++) {
		...
		$color = imagecolorallocate($im, 61, 150, 201);
		imageline($im, $i, 0, $i, $seccodedata['height'], $color);
		$c[0] += $step[0];
		$c[1] += $step[1];
		$c[2] += $step[2];
	}
...
function seccode_adulterate() {
	...
	$linex=$seccodedata['adulterate'];
	$liney=$seccodedata['adulterate'];
