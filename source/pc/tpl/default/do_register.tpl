...
<script src="[##$_SPATH.js##]formValidator.js" type="text/javascript"></script>
<script src="[##$_SPATH.js##]formValidatorRegex.js" type="text/javascript"></script>
...
$(document).ready(function(){
	...
	$("#inp_username").formValidator({onShow:"请输入用户名,只能是英文或者数字",onFocus:"用户名至少5个字符,最多15个字符",onCorrect:"该用户名可以注册"}).inputValidator({min:5,max:15,onError:"你输入的用户名非法,请确认"}).regexValidator({regExp:"username",dataType:"enum",onError:"用户名格式不正确"})
		...
		dataType : "json",
		async : true,
		url : "[##$_SCONFIG.webroot##]do-ajax-op-checkusername.html",
	...
	$("#inp_email").formValidator({onShow:"请输入邮箱",onFocus:"邮箱6-100个字符,输入正确了才能离开焦点",onCorrect:"恭喜你,你输对了",defaultValue:"@"}).inputValidator({min:6,max:100,onError:"你输入的邮箱长度非法,请确认"}).regexValidator({regExp:"^([\\w-.]+)@(([[0-9]{1,3}.[0-9]{1,3}.[0-9]{1,3}.)|(([\\w-]+.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(]?)$",onError:"你输入的邮箱格式不正确"}).ajaxValidator({
		...
		dataType : "json",
		async : true,
		url : "[##$_SCONFIG.webroot##]do-ajax-op-checkemail.html",
		success : function(data){
            return data;		
        },
		buttons: $("input[name$='regsubmit']"),
...
<div class="wrap">
  <div class="box1">
    ...
    <div class="boxcontent">
      <form id="form1" name="form1" method="post" action="[##$_SCONFIG.webroot##]do-register.html"> 
      ...
      <table style="margin:auto; float:left;">
        ...
        <tr height="40">
         ...
         <td colspan="2">
           ...
           <textarea  cols="70" rows="5" readonly="readonly" style="font-size:12px; margin-top:10px;">[##$registerrule##]</textarea>
         </td>
         </tr>
        <tr>
