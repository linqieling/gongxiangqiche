...
if(!empty($id)){
   $sql="select info.*,category.catname,category.ckeywords,category.cdescription from ".$_SC['tablepre']."pictures as info
         left join  ".$_SC['tablepre']."category as category on info.catid=category.catid
         where info.id=".$id;
   $query = $_SGLOBAL['db']->query($sql);
   $result = $_SGLOBAL['db']->fetch_array($query);
   $result['content']=htmlspecialchars_decode($result['content']);
   $mpurl=$_SCONFIG['webroot']."index-picturesshow-id-$id-catid-$_SGET[catid].html";;
