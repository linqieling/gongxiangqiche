...
function setarea(cityid, areaid) {
    ...
    switch (city) {
		...
		case "桂林" :
            ...
            "其他区", "其他区"
             ...
             break;	 
        default:
            var areaOptions = new Array("选择地区", "");
            break;
    }
	
	var areaObject = document.getElementById(areaid);
	areaObject.options.length = 0;
	areaObject.options[0] = new Option("选择地区", "");
	var j = 0;
	for(var i = 0; i < areaOptions.length/2; i++) {
		j = i + 1;
	    areaObject.options[j] = new Option(areaOptions[i*2],areaOptions[i*2+1]);
	}
}
...
function showcity(cityid, city, provinceid, boxid, area) {
...
function showarea(areaid, area, cityid, boxid) {
	...
	$(boxid).appendChild(selObj);
	if(area == "") {
		selObj.options[0] = new Option("选择地区", "");
	} else {
		selObj.options[0] = new Option(area, area);
	}

	if(cityid != '') {
		setarea(cityid, areaid);
		initprovcity(areaid, area);
	}
}
