...
function sendmail($toemail, $subject, $message, $from='') {
	global $_SC, $_SCONFIG, $_SGLOBAL, $_TPL;
	
	include_once(S_ROOT.'./config.php');//调用config文件
	
	$_TPL->assign("timestamp",$_SGLOBAL['timestamp']);
	$_TPL->assign("_SCONFIG",$_SCONFIG);
	$_TPL->assign("message",$message);	

	ob_start();

	$_TPL->display("sendmail.tpl"); 
	...
	$headers = "From: $email_from{$maildelimiter}X-Priority: 3{$maildelimiter}X-Mailer: SC_CMS ".X_VER."{$maildelimiter}MIME-Version: 1.0{$maildelimiter}Content-type: text/html; charset=$_SC[charset]{$maildelimiter}Content-Transfer-Encoding: base64{$maildelimiter}";
	...
	if($mail['mailsend'] == 1) {
		...
		fputs($fp, ($mail['auth'] ? 'EHLO' : 'HELO')." TQCMS\r\n");
		$lastmessage = fgets($fp, 512);
