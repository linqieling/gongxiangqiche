...
if(checkperm("admin_pictures",1)) {
	cpmessage('no_authority_management_operation');
}

if(!empty($id)){
  $sql="select catid from ".$_SC['tablepre']."pictures where id=".$id;
...
switch ($op){
	case 'add': 
		if(!submitcheck('submit')) {
		  ...
		  inserttable($_SC['tablepre'],"pictures", $data, 1 );
	...
	case 'edit':
		if(!submitcheck('submit')) {//如果没返回则说明进入修改界面
		  ...
		  $sql = "select pictures.*,category.groupid from ".$_SC['tablepre']."pictures as pictures left join ".$_SC['tablepre']."category as category on category.catid=pictures.catid where pictures.id=".$_GET['id'];
		  $query = $_SGLOBAL['db']->query($sql);
		  $result = $_SGLOBAL['db']->fetch_array($query);
		  $result['content'] = htmlspecialchars_decode($result['content']);
		}else{   
		  $data=data_post($_POST,$_FILES);
		  updatetable($_SC['tablepre'],'pictures',$data,'id='.$_POST['id'],0);
	...
	case 'top':
		...
		$sql="update ".$_SC['tablepre']."pictures set topdateline=".$topdateline." where id=".$_GET['id'];
		$query = $_SGLOBAL['db']->query($sql);
		cpmessage($_SESSION['lang'] == 'english'?'Operation successful!':'操作成功', $_SGLOBAL['refer']);
	break;
	case 'del':
		$sql="delete from ".$_SC['tablepre']."pictures where id=".$_GET['id'];
		$query = $_SGLOBAL['db']->query( $sql );		
	...
	break;
	case 'delpic':
		$sql = "select * from ".$_SC['tablepre']."pictures where id=".$_GET['id'];
		$query = $_SGLOBAL['db']->query($sql);
		$result = $_SGLOBAL['db']->fetch_array($query);
		$sql="update ".$_SC['tablepre']."pictures set picfilepath='' where id=".$_GET['id'];
		$query = $_SGLOBAL['db']->query( $sql );
	...
	default:
		...
		if(submitcheck('deletesubmit')){
			$ids=$_POST['ids'];
			if(!empty($ids)){
			  $sql='delete from '.$_SC['tablepre'].'pictures where 1>2 ';
			  foreach ($ids as $id){
				  $sql.=' or id ='.$id;
			  }
			  $query = $_SGLOBAL['db']->query($sql);
			}
			cpmessage($_SESSION['lang'] == 'english'?'Successfully deleted!':'删除成功', 'admin.php?view=pictures');
		}
		$search=array(
		...
		$mpurl = 'admin.php?view=pictures&sid='.$search['sid'].'&username='.$search['susername'].'&sstarttime='.$search['sstarttime'].'&sendtime='.$search['sendtime'].'&sname='.$search['sname'].'&scatid='.$search['scatid'];
		if($page<1) $page = 1;
		$start = ($page-1)*$perpage;
		//检查开始数
		ckstart($start, $perpage);
		$sql="select pictures.*,category.catname,member.username from ".$_SC['tablepre']."pictures as pictures 
			  left join  ".$_SC['tablepre']."category as category on category.catid=pictures.catid
			  left join  ".$_SC['tablepre']."members as member on member.uid=pictures.uid
			  where 1 ";
		if($search['sid']>0){
			$sql.=" and pictures.id=".$search['sid'];
		}
		if($search['scatid']>0){
			$sql.=" and pictures.catid=".$search['scatid'];
		}
		if(strlen($search['susername'])>0){
			$sql.=" and member.username='".$search['susername']."'";
		}
		if(strlen($search['sstarttime'])>0){
			$sql.=" and pictures.dateline>=".checktime($search['sstarttime']);
		}
		if(strlen($search['sendtime'])>0){
			$sql.=" and pictures.dateline<=".(checktime($search['sendtime'])+86400);
		}
		$sql.=" and pictures.name like '%".$search['sname']."%'";
		
		$query = $_SGLOBAL['db']->query($sql);
		$count=mysql_num_rows($query);
		$sql.=' order by pictures.topdateline desc, pictures.id desc limit '.$start.','.$perpage;
		$query = $_SGLOBAL['db']->query($sql);
		$datalist = array();
