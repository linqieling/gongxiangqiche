...
switch ($op){
	...
	case 'edit':
		if(!submitcheck('submit')) {//如果没返回则说明进入修改界面
		  $sql="select autoreply.*,textreply.content from ".$_SC['tablepre']."autoreply as autoreply
			    left join ".$_SC['tablepre']."textreply as textreply on textreply.replyid=autoreply.id
	...
	break;	
	case 'del':
	    $sql="select * from ".$_SC['tablepre']."textreply where replyid=".$_GET['id'];
		$query = $_SGLOBAL['db']->query($sql);
		$result = $_SGLOBAL['db']->fetch_array($query);
		if(empty($result)){
	...
	default:
		...
		if(submitcheck('deletesubmit')){
			...
			if(!empty($ids)){
			  foreach ($ids as $id){
				  ...
				  $query = $_SGLOBAL['db']->query($sql);	
				  $sql="delete from ".$_SC['tablepre']."textreply where replyid=".$id;
				  $query = $_SGLOBAL['db']->query($sql);		
			  }
			}
		...
		//开始查询
		$perpage = 15;
		$mpurl = 'admin.php?view=textreply&sid='.$search['sid'].'&sname='.$search['sname'].'&sstarttime='.$search['sstarttime'].'&sendtime='.$search['sendtime'].'&semail='.$search['semail'];	
		$page = empty($_GET['page'])?1:intval($_GET['page']);
		if($page<1) $page = 1;
		$start = ($page-1)*$perpage;
		//检查开始数
		ckstart($start, $perpage);
		$sql="select textreply.content,autoreply.*,members.username as username from ".$_SC['tablepre']."textreply as textreply
			  left join ".$_SC['tablepre']."autoreply as autoreply on autoreply.id=textreply.replyid
			  left join ".$_SC['tablepre']."members as members on members.uid=textreply.uid
		      where 1 and textreply.uid=".$_SGLOBAL['tq_uid'];
		if($search['sid']>0){
			$sql.=" and textreply.id=".$search['sid'];
		}
		if(strlen($search['susername'])>0){
			$sql.=" and member.username='".$search['susername']."'";
		}
		if(strlen($search['sstarttime'])>0){
			$sql.=" and textreply.dateline>=".checktime($search['sstarttime']);
		}
		if(strlen($search['sendtime'])>0){
			$sql.=" and textreply.dateline<=".(checktime($search['sendtime'])+86400);
		}
