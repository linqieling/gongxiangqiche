...
if(submitcheck('deletesubmit')) {
	...
	cpmessage('do_success', 'admin.php?view=backup');
	...
	if(!file_exists(S_ROOT.'./data/'.$_POST['datafile'])) {
		...
		if($fileext == 'sql') {
			cpmessage('start_transferring_data', 'admin.php?view=backup&op=import&do=import&datafile='.$_POST['datafile']);
		} elseif($fileext == 'zip') {
			cpmessage('start_transferring_data', 'admin.php?view=backup&op=import&do=zip&datafile='.$_POST['datafile']);
		} else {
...
if(empty($_GET['op'])) {
	...
	$jltablelist = fetchtablelist($_SC['tablepre']);
	...
	$idstring = '# Identify: '.base64_encode("$_SGLOBAL[timestamp],".SC_VER.",$type,$method,$volume")."\n";
	...
	if($usezip) {
		include_once S_ROOT.'./framework/class/class_zib.php';
	}
	...
	if($method == 'multivol') {//分卷备份
		...
		if(trim($sqldump)) {
			$sqldump = "$idstring".
			"# <?exit();?>\n".
			"# TQCMS Multi-Volume Data Dump Vol.$volume\n".
			"# Version: TQCMS ".SC_VER."\n".
			"# Time: $time\n".
			"# Type: $type\n".
			"# Table Prefix: $_SC[tablepre]\n".
			"#\n".
			"# TianQi Soft: http://www.gx-tq.cn\n".
			"# Please visit our website for newest infomation about TQCMS \n".
			"# ---------------------------------------------------------\n\n\n".
			"$setnames".
			$sqldump;
			...
			if(!fwrite($fp, $sqldump)) {
				fclose($fp);
				cpmessage('failure_writes_the_document_check_file_permissions', 'admin.php?view=backup');
			} else {
				fclose($fp);
				if($usezip == 2) {
			...
			if($usezip == 1){
				...
				if($zipfile->create($arrayzipfile, PCLZIP_OPT_REMOVE_PATH, S_ROOT.'./data/'.$backupdir)) {
					...
					cpmessage('complete_database_backup', 'admin.php?view=backup', 1, array($volume-1));
				}
				fclose(fopen(S_ROOT.'./data/'.$backupdir.'/index.htm', 'a'));
				cpmessage('successful_data_compression_and_backup_server_to', 'admin.php?view=backup');
			} else {
				fclose(fopen(S_ROOT.'./data/'.$backupdir.'/index.htm', 'a'));
				cpmessage('complete_database_backup', 'admin.php?view=backup', 1, array($volume-1));
			}
		...
		if(file_exists($dumpfile)) {
			...
			if($usezip) {
				...
				if($zipfile->create($dumpfile, PCLZIP_OPT_REMOVE_PATH, S_ROOT.'./data/'.$backupdir)) {
					...
					cpmessage('backup_file_compression_failure', 'admin.php?view=backup');
				}
			} else {
				fclose(fopen(S_ROOT.'./data/'.$backupdir.'/index.htm', 'a'));
				cpmessage('successful_data_compression_and_backup_server_to', 'admin.php?view=backup');
			...
			cpmessage('shell_backup_failure', 'admin.php?view=backup');
		}
	}
	...
	if($_GET['do'] == 'zip') {
		...
		if(!$_GET['confirm'] && $identify[1] != SC_VER) {
			$showform = 1;
		...
		if(!empty($_GET['multivol'])) {
			...
			if(file_exists(S_ROOT.'./data/'.$_GET['datafile'])) {
				cpmessage('decompress_data_files_success', 'admin.php?view=backup&op=import&do=zip&multivol='.$_GET['multivol'].'&datafile_vol1='.$_GET['datafile_vol1'].'&datafile='.$_GET['datafile'].'&confirm=yes', 1, array($_GET['multivol']));//debug
			} else {
				$showform = 2;
				include template('admin/tpl/backup');
				exit();
		...
		if($fp = @fopen($datafile_root, 'rb')) {
			...
			if(isset($_GET['autoimport'])) {
				cpmessage('the_volumes_of_data_into_databases_success', 'admin.php?view=backup');
			} else {
				cpmessage('data_file_does_not_exist');
			}
		}
		...
		if($identify[3] == 'multivol') {
			...
			if($identify[4] == 1) {
				...
				cpmessage('data_files_into_success', "admin.php?view=backup&op=import&do=import&datafile=$datafile_next&autoimport=yes".(isset($unzip) ? '&delunzip=yes' : ''), 1, array($identify[4]));
			} else {
				cpmessage('the_volumes_of_data_into_databases_success', 'admin.php?view=backup');
			}
