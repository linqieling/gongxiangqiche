...
switch ($op){
	case 'index':
		...
		$sql="select u.*,drive.* from ".$_SC['tablepre']."user as u 
	...
	case 'post':
		   ...
		   unset($data['file']);
		    ...
		    if($data['uid']){
		   	  $sql="select  uid  from ".$_SC['tablepre']."user_drive  where uid=".$data['uid'];
		   	  $query = $_SGLOBAL['db']->query($sql);
		      $checked = $_SGLOBAL['db']->fetch_array($query);
		      if($checked){
		      	 $data['auditor']=$_SGLOBAL['tq_uid'];
		   	     $data['updateline']=time();
		      	 updatetable($_SC['tablepre'],'user_drive',$data,'uid='.$checked['uid'],0);
		      }else{
		      	 $data['dateline']=time();
		   	     inserttable($_SC['tablepre'],"user_drive", $data,1);	
		      }
           ...
           if($data['status']=='2'|| $data['status']=='-1'){
				if($data['status']=='-1'){
					push_user_msg('2',$data['uid'],'2',$data['status'],$data['content']);
				}else{
					push_user_msg('2',$data['uid'],'2',$data['status']);
				}
			}
...
$_TPL->display("dnn_user_drive.tpl");
//wxtid-----------模板id
//uid-------------用户uid
