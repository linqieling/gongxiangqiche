...
switch ($op){
	...
	case 'cancel':
	    if($_GET['id']){
			...
			$types = $_GET['type']?$_GET['type']:0;

	        $sql = "select id,uid,status,orderno,vid,startdateline,totalmileage,dateline from ".$_SC['tablepre']."order_list where id=".$_GET['id'];
			$query = $_SGLOBAL['db']->query($sql);
	...
	case 'export':
            ...
            $sql="select o.status as ostatus,o.id,o.vid,o.duration,o.mileage,o.dateline,o.totalmoney,o.finalmoney,o.paystatus,
			  ...
			  left join  ".$_SC['tablepre']."user as u on u.uid=o.uid
			  left join  ".$_SC['tablepre']."vehicle_list as v on v.id=o.vid
			  left join  ".$_SC['tablepre']."order_returencar re  on re.oid=o.id where 1 and o.oldtype=0 ";
			$ids=$_GET['ids'];
			if(!empty($ids)){
			    $sql.="and o.id in(".$ids.")";
			 }
	...
	case "list_api":
		...
		$sql="select o.status as ostatus,o.id,o.vid,o.duration,o.mileage,o.dateline,o.totalmoney,o.paystatus,o.startdateline,o.totalmileage,o.platenumber as platenumbers,field.phone,u.nickname,u.uid as uid,v.platenumber,vs.totalmileage as vtotalmileage,re.uid as uchid,ob.dateline as bdateline from ".$_SC['tablepre']."order_list as o 
			inner join  ".$_SC['tablepre']."user as u on u.uid=o.uid 
			inner join  ".$_SC['tablepre']."user_field as field on field.uid=o.uid 
			inner join  ".$_SC['tablepre']."vehicle_list as v on v.id=o.vid 
			inner join  ".$_SC['tablepre']."vehicle_status as vs on vs.vid=o.vid 
			left join  ".$_SC['tablepre']."order_before as ob on ob.oid=o.id 
