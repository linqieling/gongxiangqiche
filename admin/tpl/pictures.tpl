...
<input type="hidden" name="view" value="pictures">
  <table class="sctable3"  width="98%" align="center" border="0" cellpadding="1" cellspacing="4" style="margin-top:20px;">
    <tr>
      <td width="70"  align="right">图集ID：</td>
...
<form method="post" action="admin.php?view=pictures&op=[##$op##]" enctype="multipart/form-data"  >
  ...
  <table class="sctable2 autocolspan" data-toggle="formlist" width="98%" align="center" border="0" cellpadding="3" cellspacing="1">
    <tr>
      <td class='title autocolspancount'>图集列表</td>
    </tr>
    <tr>
      <td class="autocolspancount" align="left">
        <input type="button" onClick="javascript:window.location.href='admin.php?view=pictures&op=add'" value="增加" class="submit">
        <input type="button" onClick="javascript:window.location.href='admin.php?view=pictures'" value="全部" class="submit">
    ...
    <tr [##if $smarty.section.loop.index is even##] class="even"[##/if##]>
      ...
      <td>
      &nbsp;<a href="admin.php?view=pictures&op=edit&id=[##$datalist[loop].id##]">编辑</a>
      [##if $datalist[loop].topdateline##]
        &nbsp;<a href="admin.php?view=pictures&op=top&id=[##$datalist[loop].id##]&top=0">取消置顶</a>
      [##else##]
        &nbsp;<a href="admin.php?view=pictures&op=top&id=[##$datalist[loop].id##]&top=1">置顶</a>
      [##/if##]
      &nbsp;<a href="admin.php?view=pictures&op=del&id=[##$datalist[loop].id##]">删除</a>
      </td>
    </tr>
    [##sectionelse##]
    <tr>
...
$(document).ready(function(){
	getcategory([##$result.groupid##],'pictures',[##$result.catid##]);
	$(".groupid").click( function() {
        groupid=$(".groupid").eq($(".groupid").index(this)).val();
		getcategory(groupid,'pictures');
	});
});
...
<table class="sctable1" align="center" width="98%" border="0" cellpadding="3" cellspacing="1" style="margin-top:20px;">
  <tr>
    <td colspan="2" class='title'>图集管理</td>
  </tr>
  <tr>
     <td width="80" align="right">图集名称：</td>   
  ...
  <tr>
     <td align="right">图集图片：</td>   
     <td align="left">[##if $result.picfilepath##]<img src="[##$_SC.attachdir##]image/[##$result.picfilepath##]" width="100" height="80" style="float:left"><a style="float:left; line-height:80px; margin-left:10px;" href="admin.php?view=pictures&op=delpic&id=[##$result.id##]">删除</a>[##else##]<input type="file" name="picfilepath"  id="poster"/>[##/if##]</td>   
  </tr>
  <tr>
