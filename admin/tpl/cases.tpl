...
<input type="hidden" name="view" value="cases">
  <table class="sctable3"  width="98%" align="center" border="0" cellpadding="1" cellspacing="4" style="margin-top:20px;">
    <tr>
      <td width="70"  align="right">案例ID：</td>
...
<form method="post" action="admin.php?view=cases&op=[##$op##]" enctype="multipart/form-data"  >
  ...
  <table class="sctable2 autocolspan" data-toggle="formlist" width="98%" align="center" border="0" cellpadding="3" cellspacing="1">
    <tr>
      <td class='title autocolspancount'>案例列表</td>
    </tr>
    <tr>
      <td class="autocolspancount" align="left">
        <input type="button" onClick="javascript:window.location.href='admin.php?view=cases&op=add'" value="增加" class="submit">
        <input type="button" onClick="javascript:window.location.href='admin.php?view=cases'" value="全部" class="submit">
    ...
    <tr [##if $smarty.section.loop.index is even##] class="even"[##/if##]>
      ...
      <td><img style="width: 80px;height: 80px;" src="[##picredirect($datalist[loop].weblogo)##]" alt=""></td>
      <td><div style="width:98%; margin:auto; text-align:left;"><a href='index-casesshow-id-[##$datalist[loop].id##]-catid-[##$datalist[loop].catid##].html' target="_blank">[##$datalist[loop].name ##]</a></div></td>
      ...
      <td>
      &nbsp;<a href="admin.php?view=cases&op=edit&id=[##$datalist[loop].id##]">编辑</a>
      [##if $datalist[loop].topdateline##]
        &nbsp;<a href="admin.php?view=cases&op=top&id=[##$datalist[loop].id##]&top=0">取消置顶</a>
      [##else##]
        &nbsp;<a href="admin.php?view=cases&op=top&id=[##$datalist[loop].id##]&top=1">置顶</a>
      [##/if##]
      &nbsp;<a href="admin.php?view=cases&op=del&id=[##$datalist[loop].id##]">删除</a>
      &nbsp;<a href="admin.php?view=cases&op=html&id=[##$datalist[loop].id##]&catid=[##$datalist[loop].catid##]">生成HTML</a>&nbsp;
      </td>
    </tr>
    [##sectionelse##]
    <tr>
...
$(document).ready(function(){
	getcategory([##$result.groupid##],'cases',[##$result.catid##]);
	$(".groupid").click( function() {
        groupid=$(".groupid").eq($(".groupid").index(this)).val();
		    getcategory(groupid,'cases');
	});
...
<table class="sctable1" align="center" width="98%" border="0" cellpadding="3" cellspacing="1" style="margin-top:20px;">
  <tr>
    <td colspan="2" class='title'>案例管理</td>
  </tr>
  <tr>
     <td width="80" align="right">案例名称：</td>   
     <td align="left" style="">
      <input name="name" type="text" class="validateMust" checkVar="案例名称不能为空"  size="30" value="[##$result.name##]" />
     </td>   
  </tr>
  <tr>
     <td width="80" align="right">案例类型：</td>   
     <td align="left" style="">
      <input name="webtype" type="text" class="validateMust" checkVar="案例类型不能为空"  size="30" value="[##$result.webtype##]" />
     </td>   
  </tr>
  <tr>
	 ...
	 [##foreach from=$_SGLOBAL.categorygroup name=list item=list##]
       <input type="radio" class="groupid" name="groupid" [##if $list.id==$result.groupid##]checked="checked"[##/if##] [##if !$list@first##]margin-left:10px;[##/if##] value="[##$list.id##]">
  ...
  <tr>
     ...
     <td align="left"><textarea name="keywords" cols="100" rows="3">[##$result.keywords##]</textarea>(帮助增加搜索引擎收录，多个关键字用,隔开)</td>   
  </tr>
  <tr>
     <td align="right">description：</td>   
     <td align="left">
     <textarea name="description" cols="100" rows="3">[##$result.description##]</textarea>
  </tr>
  <tr>
     <td align="right">案例摘要：</td>   
     <td align="left">
       <textarea name="brief" cols="100" rows="5">[##$result.brief##]</textarea>
     </td>   
  </tr>
  <tr>
     <td align="right">案例内容：</td>   
     <td align="left">
  ...
  <tr>
     <td align="right">案例logo：</td>   
     <td align="left">[##if $result.weblogo##]<img src="[##$_SC.attachdir##]image/[##$result.weblogo##]" width="100" height="80" style="float:left"><a style="float:left; line-height:80px; margin-left:10px;" href="admin.php?view=cases&op=delpic&id=[##$result.id##]&refer=[##$_SGLOBAL.refer##]">删除</a>[##else##]
     <a href="javascript:;" class="a-upload">
     <input type="file" name="weblogo"  id="poster"/>
  ...
  <tr>
     ...
     <td align="left">[##if $result.picfilepath##]<img src="[##$_SC.attachdir##]image/[##$result.picfilepath##]" width="100" height="80" style="float:left"><a style="float:left; line-height:80px; margin-left:10px;" href="admin.php?view=cases&op=delpic&id=[##$result.id##]&refer=[##$_SGLOBAL.refer##]">删除</a>[##else##]
     <a href="javascript:;" class="a-upload">
  ...
  <tr>
     <td width="80" align="right">案例日期：</td>   
     <td align="left" style="">
      <input name="dateline" type="text"  size="30" value="[##$result.dateline|date_format:'%Y-%m-%d'##]" />
     </td>   
  </tr>
  <tr>
     <td width="80" align="right">案例链接：</td>   
     <td align="left" style="">
      <input name="url" type="text"  style="width: 100%" value="[##$result.url##]" />
     </td>   
  </tr>
  <tr>
     <td align="right">审核：</td>   
     <td align="left">
      <input type="radio" name="pass" style="vertical-align: middle;" value="1" [##if $result.pass eq 1##] checked [##/if##]>
      <label>是</label>
      <input type="radio" name="pass" style="margin-left:10px;vertical-align: middle;" value="0" [##if $result.pass eq 0##] checked [##/if##]>
      <label>否</label>
     </td>   
  </tr>   
</table>
