    ...
    <tr>
      ...
      <td width="165" align="left">
        <select name='scatid' class="catid">
          <option value='0'>==请选择下载分类==</option>
          [##foreach from=$_SGLOBAL.category name=list item=list##]
              [##if $list.modname eq 'down'##]
...
<form method="post" action="admin.php?view=down">
  ...
  <table class="sctable2 autocolspan" data-toggle="formlist" width="98%" align="center" border="0" cellpadding="3" cellspacing="1">
    <tr>
      <td class='title autocolspancount'>下载列表</td>
    </tr>
    <tr>
      <td class="autocolspancount" align="left">
        <input type="button" onClick="javascript:window.location.href='admin.php?view=down&op=add'" value="增加" class="submit">
        <input type="button" onClick="javascript:window.location.href='admin.php?view=down'" value="全部" class="submit">
    ...
    <tr [##if $smarty.section.loop.index is even##] class="even"[##/if##]>
      ...
      <td><div style="width:98%; margin:auto; text-align:left;"><a href='index-downshow-id-[##$datalist[loop].id##]-catid-[##$datalist[loop].catid##].html' target="_blank">[##$datalist[loop].name ##]</a></div></td>
      ...
      <td>
      &nbsp;<a href="admin.php?view=down&op=edit&id=[##$datalist[loop].id##]">编辑</a>
      [##if $datalist[loop].topdateline##]
        &nbsp;<a href="admin.php?view=down&op=top&id=[##$datalist[loop].id##]&top=0">取消置顶</a>
      [##else##]
        &nbsp;<a href="admin.php?view=down&op=top&id=[##$datalist[loop].id##]&top=1">置顶</a>
      [##/if##]
      &nbsp;<a href="admin.php?view=down&op=del&id=[##$datalist[loop].id##]">删除</a>
      &nbsp;<a href="admin.php?view=down&op=html&id=[##$datalist[loop].id##]&catid=[##$datalist[loop].catid##]">生成HTML</a>&nbsp;
      </td>
    </tr>
    [##sectionelse##]
    <tr> 
...
$(document).ready(function(){
    var urlbox_max=[##$urlcount##];
	$("#addurl").click(function (){addurl();});
	function addurl(){	
		if(urlbox_max<=4){
		   $(".inputurl").append("<div id=urlbox_"+urlbox_max+">标题:<input name='downtitle_"+urlbox_max+"' size='15'  type='text' value='其他下载点链接'/>&nbsp;&nbsp;链接地址:<input name='downurl_"+urlbox_max+"' size='25'  type='text'  class=picfile />&nbsp;&nbsp;<input type='button' value='删除' class=submit id='delpic_"+urlbox_max+"'/></div>");
		   $("#delpic_"+urlbox_max).bind('click', function(){
			   $(this).parent().remove();
			   urlbox_max=urlbox_max-1;
		   });
		   urlbox_max=urlbox_max+1;
		   $("#url_num").val(urlbox_max);
		}else{
		   alert('您最多只能添加5个链接!');
		}
	}
...
<script type="text/javascript">
$(document).ready(function(){
	getcategory([##$result.groupid##],'down',[##$result.catid##]);
	$(".groupid").click( function() {
        groupid=$(".groupid").eq($(".groupid").index(this)).val();
		getcategory(groupid,'down');
	});
});
...
<form method="post" action="admin.php?view=down&op=[##$op##]" enctype="multipart/form-data" >
<input type="hidden" name="formhash" value="[##$_SGLOBAL.formhash##]" />
<input type="hidden" name="refer" value="[##$_SGLOBAL.refer##]" />
<input type="hidden" name="id"  value="[##$result.id##]" />
<table class="sctable1" align="center" width="98%" border="0" cellpadding="3" cellspacing="1" style="margin-top:20px;">  
  <tr>
    <td colspan="2" class='title'>下载管理</td>
  </tr>
  <tr>
     <td width="80" align="right">下载名称：</td>   
  ...
  <tr>
     <td align="right">本地链接：</td>   
     <td align="left">
     [##if $result.downurl eq ''##] 
       <input type="file" name="poster"  />
     [##else##]
       <a href="[##$_SC.attachdir##]file/[##$result.downurl##]">下载</a> 
       <a href="admin.php?view=down&op=dellurl&id=[##$result.id##]">删除</a>
     [##/if##]
     </td>   
  </tr>
   <tr>
     <td align="right">其他链接：</td>   
     <td align="left"> 
     <input type="button" name="addurl" value="添加链接" id="addurl" class="submit" />&nbsp;      
     <div class="inputurl"></div>
     [##section name=loop loop=$urlarray##]
     <div style="width:780px; height:30px; line-height:30px;">
       <div style="float:left; margin-left:10px; width:20px">[##$smarty.section.loop.index+1##].</div> 
       <div style="float:left; width:200px;">链接标题:[##$urlarray[loop].downtitle##]</div> 
       <div style="float:left; width:400px;">链接地址:[##$urlarray[loop].downurl##]</div> 
       <div style="float:left; width:100px;"><a href="admin.php?view=down&op=delourl&id=[##$result.id##]&oid=[##$urlarray[loop].id##]">删除</a></div>
     </div>   
     [##/section##]
     <input type="hidden" name="url_num" value="[##$urlcount##]" id="url_num"/>
     </td>   
  </tr> 
</table>
